...
export const validateAvailableLimit =
  ...
  (_: unknown, value: number): Promise<void> => {
    if (!value) {
      return Promise.reject(new Error(t('dashboard.forms.limit.fields.availableLimit')));
    }
...
export const validateDailyLimit =
  ...
  (_: unknown, value: number): Promise<void> => {
    ...
    if (!value) {
      return Promise.reject(new Error(t('dashboard.forms.limit.fields.dailyLimit')));
    }
