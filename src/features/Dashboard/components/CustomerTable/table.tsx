...
const toClassSafe = (value?: string) =>
  value?.toString().toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'unknown';
...
export const customerColumns = (
  ...
  },
  {
    ...
    render: (_, record) => (
      <div className="dashboard-table__cell">
        <p className="dashboard-table__title">{record.phone || t('dashboard.table.na')}</p>
  ...
  },
  {
    ...
    render: (_, record) => (
      <div className="dashboard-table__cell">
        ...
        <span className="dashboard-table__meta">
          {record.dateOfBirth ? formatDate(record.dateOfBirth) : t('dashboard.table.na')}
        </span>
      </div>
    ),
  },
  {
    ...
    render: (_, record) => (
      <div className="dashboard-table__cell">
        <span className={`dashboard-table__tag dashboard-table__tag--${toClassSafe(record.kycStatus)}`}>
        ...
        >
          {record.isActive ? t('dashboard.status.active') : t('dashboard.status.inactive')}
        </span>
      </div>
    ),
  },
  {
    ...
    render: (_, record) => (
      <div className="dashboard-table__cell">
        <p className="dashboard-table__title">{formatAddress(record.address)}</p>
        <span className="dashboard-table__meta">
  ...
  },
  {
    ...
    render: (_, record) => (
      <div className="dashboard-table__cell">
        <p className="dashboard-table__title">
          {record.updatedAt ? formatDate(record.updatedAt) : t('dashboard.table.na')}
        </p>
        <span className="dashboard-table__meta">
          {t('dashboard.table.createdAt', {
            value: record.createdAt ? formatDate(record.createdAt) : t('dashboard.table.na'),
          })}
        </span>
      </div>
    ),
